...
(defconst hilit-default-face-table
  '(
    ...
    (comment	firebrick-italic-bold    moccasin           italic)
    (include	purple-bold		    Plum1	       bold-italic)
    (define	ForestGreen-bold    green	       bold)
    (defun	blue-bold	    cyan-bold	       bold-italic)
    (decl	RoyalBlue-bold	    cyan	       bold)
    (type	nil		    yellow	       nil)
    (keyword	RoyalBlue-bold	    cyan	       bold-italic)
    (label	red-underline	    orange-underlined  underline)
    (string	red-bold		    orange	       underline)
    
    ;; some further faces for Ada
    (struct	  black-bold        white-bold	       bold)
    (glob-struct  magenta-bold	    Plum1	       default-bold-underline)
  ...
  (hilit-set-mode-patterns
   ...
   (append
    ...
    '(
      ...
      ;; key words
      ("[^_]\\<\\(auto\\|int\\|extern\\|typedef\\|void\\|while\\|for\\|do\\|struct\\|static\\|void\\|if\\|else\\|return\\|continue\\|default\\|goto\\|case\\|switch\\|char\\|unsigned\\|signed\\|union\\|short\\|long\\|const\\|break\\|enum\\|float\\|double\\)\\>[^_]" 1 keyword)
      )))
...
(let ((comments     '(("/\\*" "\\*/" comment)))
  ...
  (hilit-set-mode-patterns
   'java-mode
   (append
    comments java-comments strings preprocessor
    '(
      ;; function decls are expected to have types on the previous line
      ("^\\(\\(\\w\\|[$_]\\)+::\\)?\\(\\w\\|[$_]\\)+\\s *\\(\\(\\w\\|[$_]\\)+\\s *((\\|(\\)[^)]*)+" nil defun)
      ("^\\(\\(\\w\\|[$_]\\)+[ \t]*::[ \t]*\\)?\\(\\(\\w\\|[$_]\\)+\\|operator.*\\)\\s *\\(\\(\\w\\|[$_]\\)+\\s *((\\|(\\)[^)]*)+" nil defun)
      ("^\\(template\\|typedef\\|struct\\|union\\|class\\|enum\\|public\\|private\\|protected\\).*$" nil decl)
      ;; datatype -- black magic regular expression
      ("[ \n\t({]\\(\\(final\\|register\\|volatile\\|unsigned\\|extern\\|static\\)\\s +\\)*\\(\\(\\w\\|[$_]\\)+_t\\|float\\|double\\|void\\|char\\|String\\|short\\|int\\|boolean\\|long\\|byte\\|\\(\\(struct\\|union\\|enum\\|class\\)\\([ \t]+\\(\\w\\|[$_]\\)*\\)\\)\\)\\(\\s +\\*+)?\\|[ \n\t;()]\\)" nil type)
      ;; key words
      ("[^_]\\<\\(return\\|goto\\|if\\|try\\|catch\\|else\\|case\\|default\\|switch\\|break\\|continue\\|while\\|do\\|for\\|public\\|protected\\|private\\|delete\\|new\\|true\\|false\\)\\>[^_]"
       1 keyword)))))
