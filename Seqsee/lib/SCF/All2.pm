{
  ...
  sub run {
    ...
    my $obj = $opts_ref->{obj}
    // confess "Needed 'obj', only got " . join( ';', keys %$opts_ref );
    my $cat = $opts_ref->{cat}
...
#main::message("Found extension: $extension; " . $extension->get_structure_string());
    ...
    eval { $extend_success = $object->Extend( $extension, $add_to_end_p ); };
    if ( my $err = $EVAL_ERROR ) {
      CATCH_BLOCK: {
        if ( UNIVERSAL::isa( $err, 'SErr::CouldNotCreateExtendedGroup' ) ) {
          my $msg = "Extending object: " . $object->as_text() . "\n";
          $msg .=
            "Extension: "
          . $extension->as_text()
          . " in direction $add_to_end_p\n";
          print STDERR $msg;
          main::message($msg);
          last CATCH_BLOCK;
        }
...

{
  ...
  sub run {
    my ( $action_object, $opts_ref ) = @_;
    my $actual = $opts_ref->{actual}
    // confess "Needed 'actual', only got " . join( ';', keys %$opts_ref );
    my $intended = $opts_ref->{intended}
    // confess "Needed 'intended', only got " . join( ';', keys %$opts_ref );
