
{
  ...
  sub run {
    ...
    my $exception = $opts_ref->{exception}
    // confess "Needed 'exception', only got " . join( ';', keys %$opts_ref );
    my $expected_object = $opts_ref->{expected_object}
    // confess "Needed 'expected_object', only got "
    . join( ';', keys %$opts_ref );
    my $start_position = $opts_ref->{start_position}
    // confess "Needed 'start_position', only got "
    . join( ';', keys %$opts_ref );
    my $known_term_count = $opts_ref->{known_term_count}
    // confess "Needed 'known_term_count', only got "
    . join( ';', keys %$opts_ref );
