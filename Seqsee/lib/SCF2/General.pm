...

{
  ...
  sub run {
    ...
    if ( my $err = $EVAL_ERROR ) {
      CATCH_BLOCK: {
        if ( UNIVERSAL::isa( $err, 'SErr::ConflictingGroups' ) ) {
          return;
          last CATCH_BLOCK;
        }
        die $err;
...

{
  ...
  sub run {
    ...
    my $transform = $opts_ref->{transform}
    // confess "Needed 'transform', only got " . join( ';', keys %$opts_ref );
...

{
  ...
  sub run {
    ...
    if ( my $err = $EVAL_ERROR ) {
      CATCH_BLOCK: {
        if ( UNIVERSAL::isa( $err, 'SErr::HolesHere' ) ) {
          return;
          last CATCH_BLOCK;
        }
        die $err;
...

{
  ...
  sub run {
    my ( $action_object, $opts_ref ) = @_;
    my $first = $opts_ref->{first}
    // confess "Needed 'first', only got " . join( ';', keys %$opts_ref );
    my $second = $opts_ref->{second}
    // confess "Needed 'second', only got " . join( ';', keys %$opts_ref );
    my $third = $opts_ref->{third}
    // confess "Needed 'third', only got " . join( ';', keys %$opts_ref );
    ...
    if ( $t1 and $t1 eq $t2 ) {
      ...
      SCat::OfObj::Alternating->CheckForAlternation( $first, $second, $third )
      or return;
    }
    SCodelet->new(
      "CreateGroup",
...

{
  ...
  sub run {
    ...
    // confess "Needed 'core', only got " . join( ';', keys %$opts_ref );
    my $direction = $opts_ref->{direction}
    // confess "Needed 'direction', only got " . join( ';', keys %$opts_ref );
  ...
  sub EstimateAskability {
    ...
    if ( SWorkspace->GetSuperGroups($end1)
      or SWorkspace->GetSuperGroups($end2) )
    {
      $supergroup_penalty = 0.6;
    }

    my $transform_activation =
    SLTM::GetRealActivationsForOneConcept($transform);
...

{
  ...
  sub run {
    ...
    // confess "Needed 'object', only got " . join( ';', keys %$opts_ref );
    my $direction = $opts_ref->{direction}
    // confess "Needed 'direction', only got " . join( ';', keys %$opts_ref );
    ...
    my $result_of_something_like = SWorkspace->LookForSomethingLike(
      {
    ...
    if ( $Global::Feature{AllowSquinting} ) {
      ...
      my $literally_present = $result_of_something_like->get_literally_present()
      or return;
