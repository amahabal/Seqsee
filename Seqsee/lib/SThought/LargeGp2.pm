...
;
            ...
            SCodelet->new("MaybeStartBlemish", 
                         100,
                         { group => $group })->schedule(); 
;
        }
            ...
            if ($extension) {
                ...
                if ( $transform->isa("Transform::Structural") ) {
                    ...
                    if ( $cat->get_name() =~ m#^Interlaced_(.*)#o ) {
                        SCodelet->new("InterlacedInitialBlemish", 
                         100,
                         {
                                count => $1,
                                group => $group,
...
;
                ...
                if ($flush_right) {
                    SCodelet->new("ArbitraryInitialBlemish", 
                         100,
                         { group => $group })->schedule(); 
;
                }
...
sub run{
    my ( $action_object, $opts_ref ) = @_;
    	my $count = $opts_ref->{count} // confess "Needed 'count', only got " . join(';', keys %$opts_ref);
	my $group = $opts_ref->{group} // confess "Needed 'group', only got " . join(';', keys %$opts_ref);
