...
;
        ...
        sub ExtendFromMemory {
            ...
            if ($flush_right and ($flush_left or $SWorkspace::ElementCount <= 3)) {
                ...
                push @actions_ret, SCodelet->new("FindIfRelated", 
                                                           1000,
                                                           {a => $core, b=>$next}); 
;
                return @actions_ret;
        ...
        sub AddCategoriesFromMemory {
            ...
            if ($weighted_set->is_not_empty()) {
                ...
                push @actions_ret, SCodelet->new("CheckIfInstance", 
                                                           100,
                                                           { obj => $core, cat => $category}); 
;
            }
            return @actions_ret;
        };
...
;
        ...
        if ($flush_left or SUtil::toss(0.8)) {
            push @actions_ret, SCodelet->new("AttemptExtensionOfGroup", 
                                                           80,
                                                           {
                    object    => $core,
                    direction => DIR::RIGHT(),
...
;
        ...
        if ( scalar(@$core) > 1 and SUtil::toss(0.8) ) {
            ...
            if ( SUtil::toss(0.5) ) {
                ...
                push @actions_ret, SCodelet->new("ConvulseEnd", 
                                                           $urgency,
                                                           {
                    object    => $core,
                    direction => $DIR::RIGHT,
...
;
        ...
        if ( $span_fraction > 0.5 ) {
            push @actions_ret, SCodelet->new("LargeGroup", 
                                                           100,
                                                           { group => $core }); 
;
        }
        ...
        if ($category and SUtil::toss(0.5 * SLTM::SpikeBy(5, $category))) {
            push @actions_ret, SCodelet->new("FocusOn", 
                                                           100,
                                                           {what => $category}); 
;
        }

        push @actions_ret, SCodelet->new("LookForSimilarGroups", 
                                                           20,
                                                           { group => $core }); 
;
        push @actions_ret, SCodelet->new("CleanUpGroup", 
                                                           20,
                                                           { group => $core }); 
;
        ...
        if ($underlying_reln) {
            ...
            if ($transform->isa('Transform::Structural')
                ...
                push @actions_ret, SCodelet->new("DoTheSameThing", 
                                                           100,
                                                           { transform => $transform }); 
;
            }
        ...
        if (my $downslope = IsThisAMountainUpslope($core)) {
            IFUPSLOPE: {
                  ...
                  push @actions_ret, SCodelet->new("CreateGroup", 
                                                           100,
        ...
        if ($Global::Feature{Alternating}) {
            ...
            if ($left_neighbour and $right_neighbour) {
                if (my ($cat) = $core->get_common_categories($left_neighbour, $right_neighbour)) {
                    push @actions_ret, SCodelet->new("CheckIfAlternating", 
                                                           100,
...
;
        ...
        sub as_text {
            my $self = shift;
             return "Group " . $self->get_core()->as_text 
        }
...
;
    ...
    ;
       ...
       my ( $self, $id, $opts_ref ) = @_;
       	my $core = $core_of{$id} = $opts_ref->{core} or confess "Missing required argument core";
	my $magnitude = $magnitude_of{$id} = ($opts_ref->{magnitude} || 0);
;
...
;
        ...
        sub as_text {
            my $self = shift;
             return "Element " . $self->get_core()->as_text()  
        }
