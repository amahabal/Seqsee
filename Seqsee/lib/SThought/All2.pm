    ...
    my $a_core = $a->can('get_core') ? $a->get_core() : undef;
        ...
        if ( $a_core and $b_core ) {
            if ( $a_core->isa("SObject") and $b_core->isa("SObject") ) {
                SAction->new({ family => "FindIfRelated", 
                                        urgency => 100,
                                        args =>  {
                    a => $a_core,
                    b => $b_core
...
;
                ...
                SAction->new({ family => "FindIfRelatedRelations", 
                                        urgency => 100,
                                        args =>  {
                    a => $a_core,
                    b => $b_core
            ...
            and ( $span / $total_count ) > 0.8 )
        {
            if ( $left_edge == 0 ) {
                if ( $span == $total_count ) {
                    ...
                    SAction->new({ family => "AttemptExtensionOfGroup", 
                                        urgency => 80,
                                        args =>  {
                        object    => $gp,
                        direction => DIR::RIGHT()
...
my $LastSolutionDescriptionTime;
        ...
        sub BelieveDone {
            ...
            SAction->new({ family => "DescribeSolution", 
                                        urgency => 100,
                                        args =>  { group => $group } })->conditionally_run(); 
;
        }
