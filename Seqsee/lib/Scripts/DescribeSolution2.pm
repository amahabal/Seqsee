...
sub run{
    ...
    if ( exists $args_ref->{__S_T_A_C_K__} ) {
     ...
     if ( $step_ == 1 ) { my $ruleapp = $group->get_underlying_reln();
        unless ($ruleapp) {
            ...
            
 {
    ...
    ; $step_++;} if ( $step_ == 2 ) { if ( my $ruleapp = $group->get_underlying_reln() ) {
        ...
        main::message( "I will describe the solution now!", 1 );
        
           {
             ...
             SCodelet->new('DescribeInitialBlemish', 10000, {
                  __S_T_E_P__ => 1,
    ...
    ; $step_++;} if ( $step_ == 3 ) { 
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeBlocks', 10000, {
                  __S_T_E_P__ => 1,
                  __A_R_G_S__ => { group => $group },
                  __S_T_A_C_K__ => $new_stack})->schedule();
             return;
        ...
        ;
    ; $step_++;} if ( $step_ == 4 ) { my $ruleapp = $group->get_underlying_reln();
        my $rule    = $ruleapp->get_rule();
        
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeRule', 10000, {
                  __S_T_E_P__ => 1,
...
sub run{
    ...
    if ( exists $args_ref->{__S_T_A_C_K__} ) {
     ...
     if ( $step_ == 1 ) { if ( my $le = $group->get_left_edge() ) {
            my @initial_bl =
              map { $_->get_mag() }
              ( SWorkspace::GetElements() )[ 0 .. $le - 1 ];
            main::message(
...
sub run{
	...
	my $ruleapp = $opts_ref->{ruleapp} // confess "Needed 'ruleapp', only got " . join(';', keys %$opts_ref);

     if ( $step_ == 1 ) { main::debug_message( "Rule is $rule", 1 );
        my $reln = $rule->get_transform;
        
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeTransform', 10000, {
                  __S_T_E_P__ => 1,
...
sub run{
    ...
    if ( exists $args_ref->{__S_T_A_C_K__} ) {
        ...
        ( $stack, $step_, $opts_ref ) = ( [], 1, $args_ref );
     	...
     	my $reln = $opts_ref->{reln} // confess "Needed 'reln', only got " . join(';', keys %$opts_ref);
	my $ruleapp = $opts_ref->{ruleapp} // 0;

     if ( $step_ == 1 ) { if ( $reln->isa('Transform::Structural') ) {
            
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeRelationCompound', 10000, {
                  __S_T_E_P__ => 1,
                  __A_R_G_S__ => { reln => $reln, ruleapp => $ruleapp },
                  __S_T_A_C_K__ => $new_stack})->schedule();
             return;
        ...
        ;
           ...
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeRelationSimple', 10000, {
                  __S_T_E_P__ => 1,
        ...
        ;
            ...
            main::message( "Strange bond! Something wrong, let abhijit know",
                1 );
        }
    ; $step_++;} if ( $step_ == 2 ) { 
 {
...
sub run{
	...
	my $ruleapp = $opts_ref->{ruleapp} // confess "Needed 'ruleapp', only got " . join(';', keys %$opts_ref);
     ...
     if ( $step_ == 1 ) { my $category = $reln->get_category();
        
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeRelnCategory', 10000, {
                  __S_T_E_P__ => 1,
                  __A_R_G_S__ => {
            cat     => $category,
            ruleapp => $ruleapp,
        ...
        ;
    ; $step_++;} if ( $step_ == 2 ) { my $meto_mode = $reln->get_meto_mode();
        my $meto_reln = $reln->get_metonymy_reln();
        
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeRelnMetoMode', 10000, {
                  __S_T_E_P__ => 1,
                  __A_R_G_S__ => {
            meto_mode => $meto_mode,
            meto_reln => $meto_reln,
...
sub run{
	...
	my $ruleapp = $opts_ref->{ruleapp} // confess "Needed 'ruleapp', only got " . join(';', keys %$opts_ref);
     ...
     if ( $step_ == 1 ) { if ( $cat->isa('SCat::OfObj::Interlaced') ) {
            
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeInterlacedCategory', 10000, {
                  __S_T_E_P__ => 1,
                  __A_R_G_S__ => {
                cat     => $cat,
                ruleapp => $ruleapp,
...
sub run{
	...
	my $ruleapp = $opts_ref->{ruleapp} // confess "Needed 'ruleapp', only got " . join(';', keys %$opts_ref);
     ...
     if ( $step_ == 1 ) { my $parts = $cat->get_parts_count();
        if ( $parts == 2 ) {
            
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('Describe2InterlacedCategory', 10000, {
                  __S_T_E_P__ => 1,
                  __A_R_G_S__ => {
                cat     => $cat,
                ruleapp => $ruleapp,
        ...
        ;
           ...
           {
             my $new_stack = [ @$stack, [$step_ + 1, $opts_ref, $package_name_ ]];
             SCodelet->new('DescribeMultipleInterlacedCategory', 10000, {
                  __S_T_E_P__ => 1,
                  __A_R_G_S__ => {
                cat     => $cat,
                ruleapp => $ruleapp,
...
sub run{
    ...
    if ( exists $args_ref->{__S_T_A_C_K__} ) {
        ...
        ( $stack, $step_, $opts_ref ) = ( [], 1, $args_ref );
     	...
     	my $meto_mode = $opts_ref->{meto_mode} // confess "Needed 'meto_mode', only got " . join(';', keys %$opts_ref);
	my $meto_reln = $opts_ref->{meto_reln} // confess "Needed 'meto_reln', only got " . join(';', keys %$opts_ref);
	my $ruleapp = $opts_ref->{ruleapp} // confess "Needed 'ruleapp', only got " . join(';', keys %$opts_ref);

     if ( $step_ == 1 ) { unless ( $meto_mode->is_metonymy_present ) {
...
'I am squinting in order to see the blocks as instances of that category',
        ...
        my @items = @{ $ruleapp->get_items() };
        my @to_describe =
          ( scalar(@items) > 3 ) ? ( @items[ 0 .. 2 ] ) : @items;
