...
sub CreateDisplay {
    ...
    my @position_text = defined($execute_position) ? ("[$creation_position/$execute_position] ", ['execute_position']) : ("[$creation_position/xxxxx]", "wasnt_executed");
...
sub PrintProgeny {
    ...
    $text->insert('end', q{   } x $depth, '', CreateDisplay($object, $execute_position, $Details{$object}), "\n");
