...
MAIN: while (1) {
        print
            "Enter the sequence fragments, one per line. Separate each element therein by a space. End by a solitary . on a line by itself. Type done now if you want to stop.\n";
        my @input = ();
        my $in;
        while (1) {
            $in = <STDIN>;
...
sub process_test {
    ...
    my $object =
        ( $input =~ /\(/ )
        ? SBuiltObj->new_from_string($input)
        : SBuiltObj->new_deep(
...
sub SCat::generate_secondary_blemish_cats {
    ...
    my @num_blemishes = map { scalar( @{ $_->get_where } ) } @$bindings_ref;
...
# XXX that is not quite accurate: for a blemish like double all 3s, the number of blemishes may be vastly different
    ...
    #print "FORWARD:\n";
    my @forward_pos = map {
        $bindings_ref->[$_]->describe_position( "forward", $obj_ref->[$_] );
    } ( 0 .. $object_count - 1 );
    ...
    my @backward_pos = map {
        $bindings_ref->[$_]->describe_position( "backward", $obj_ref->[$_] );
    } ( 0 .. $object_count - 1 );
    ...
    my @the_pos = map {
        $bindings_ref->[$_]->describe_position( "the", $obj_ref->[$_] );
    } ( 0 .. $object_count - 1 );
    $oddness = find_odd(@the_pos);
    if ($oddness) {
        SOddman::printLn(
            "I see some differences based on what it is that is blemished");
        my $new_cat
            = $self->derive_blemish_position( $oddness->{repeated_value} );
...
sub SBindings::describe_position {
    ...
    @where == 1
        or confess
        "This half baked function should not have been called when the number of blemishes is "
        . scalar(@where);
    if ( $string eq "forward" ) {
        ...
        my $pos
            = SPos->new_the( $S::literal->build( { structure => $what } ) );
    ...
    for my $cat (@$cat_ref) {
        ...
        if ($oddness) {
            if ( $oddness->{repeated_value} == 1 ) {
                ...
                SOddman::showWhatsOdd( $what_str, $cat->get_name() );
                return $cat;
            }
            else {
                my $pos = $oddness->{odd_position};
                SOddman::appearsButNot(
...
sub SCat::generate_secondary_cats {
    ...
    for my $att (@att) {
        ...
        push @{ $posn_assumption_hash{ $oddness->{odd_position} } }, $att,
            $oddness->{repeated_value};
    }
    if (%posn_assumption_hash) {
        return
            map { $self->derive_assuming( {@$_} ) }
