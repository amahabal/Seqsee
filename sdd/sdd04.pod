...
Seqsee Design Doc 4 - Codelet System
...
implements the Coderack.

=item F<Tk::SCoderack.pm> 

implements the L<GUI|SGUI>

=item F<SCodelet.pm> 

implements the codelets.

=item Codelet Families

=over

=item F<SCF.list>

is a plain text file with the names of the code families to load. If a line in this file starts with a C<#> then that line is ignored. Each entry looks like this: C<< SCF::bond_scout >>.  So it is not really a filename.
...
=head1 Launching Codelets

A codelet is added to the coderack by the call C<< SApp::post_cc(poster, postee, options) >>, where C<poster> is the name of the poster, and C<postee> is the name of what is being posted. It is not necessary that the posting always happens, or with what urgency: the method post_cc intervenes and probabilistically posts the codelet. The codelet, if posted, will have the extra arguments as a hashref, so that the C<run()> method of the codelet can start: C<< my ($opts) = shift; >>

=head1 Configuring Behaviour

The files F<SCodeConfig.txt> and F<GenCL.pl> together create a file containing perl-code that controls what happens in a call to C<SApp::post_cc>.
