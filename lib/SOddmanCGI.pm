...
sub SOddman::attToDistinguish{
  my ( $att ) = @_;
  print "It appears that the attribute '$att' can be used to find the odd man<br>\n";
}
...
sub SOddman::appearsButNot{
  my ( $what_str, $catname ) = @_;
  SOddman::fragment( $what_str );
  print " can be construed to be an odd man out: it is the only one that is an instance of the category ";
  SOddman::category( $catname );
...
sub SOddman::Display_is_instance{
  ...
  print "<li> "; SOddman::fragment( $string );
  if ($bindings) {
