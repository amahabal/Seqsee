...
CodeletFamily FindIfMetonyable( $object !, $category ! ) does {
INITIAL: {
...
CodeletFamily FindIfGroupable( $category !, $items ! ) does {
INITIAL: {
...
## Got here in FindIfGroupable
        ...
        if ( $anchored_count == scalar(@anchored_p) ) {
            my @unstarred_items = map {
                my $unstarred = $_->get_is_a_metonym();
                if ($unstarred) {
            ...
            SErr->throw(
                "There are some unanchored and some anchored objects that were passed to me. There is a serious flaw somewhere"
            );
        }
