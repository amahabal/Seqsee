...
sub generate_instancer {
    ...
    foreach (@atts) {
        croak
            "cannot generate instancer: do not know how to guess attribute $_"
...
sub compose {
    ...
    unless ( $instancer_of{$ident} ) {
        ...
        for ( $att_of{$ident}->members ) {
            ...
            {
                croak
                    "Cannot generate guesser for $_: need at least a guesser_pos";
            }
            if ( exists $guesser_pos_of_of{$ident}{$_} ) {
                $self->generate_guesser_from_pos( $_,
                    $guesser_pos_of_of{$ident}{$_} );
            }
            else {
