    ...
    my $numeric_FindTransorm = sub {
        *__ANON__ = "((__ANON__ FindMapping SInt/SElement SInt/SElement))";
        ...
        if (grep {not defined $_} @common_categories) {
            confess "undef in common_categories FindMapping SInt/SElement SInt/SElement:".join(', ', @common_categories);
        }
...
multimethod ApplyMapping => qw(Mapping::Numeric SInt) => sub {
    *__ANON__ = "((__ANON__ ApplyMapping Mapping::Numeric SInt))";
    my ( $transform, $num ) = @_;
    my $new_mag = $transform->get_category()->ApplyMappingForCat( $transform, $num->get_mag() )
        // return;
    SInt->new($new_mag);
};
