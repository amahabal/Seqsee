...
sub setup {
    ...
    $Coderack = $tmp_frame->SCoderack( %{ $config{coderack} } )->pack( -side => "top" );

    $tmp_frame = $main_frame->Frame()->pack( -side => 'left' );
    $config{stream}{-tags_provided} = tags_to_aref( $config{stream_tags} );
    $tmp_frame->Label( %{ $config{labels} }, -text => "Stream" )->pack( -side => 'top' );
    $Stream = $tmp_frame->SStream( %{ $config{stream} } )->pack( -side => "top" );

    $tmp_frame = $main_frame->Frame()->pack( -side => 'left' );
    $config{components}{-tags_provided} = tags_to_aref( $config{components_tags} );
    $tmp_frame->Label( %{ $config{labels} }, -text => "Components" )->pack( -side => 'top' );
    $Components = $tmp_frame->SComponents( %{ $config{components} } )->pack( -side => "top" );

    $config{info}{-tags_provided} = tags_to_aref( $config{info_tags} );
    $Info = $bottom_frame->SInfo( %{ $config{info} } )->pack( -side => "left" );
