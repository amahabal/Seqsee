...
my @CATEGORIES = ( $S::descending, $S::ascending, $S::mountain, );

my @BLEMISHES = ( $S::double, );
...
sub get_change_based_on {
    ...
    my $location_change = calculate_location_change( $bindings1, $bindings2 );
    ...
    if ( $o1->get_blemished ) {
        if ( $o2->get_blemished ) {
            ...
            for ( my $i = 0; $i < @$where1_ref; $i++ ) {
                my $reln
                    = get_relation( $where1_ref->[$i], $where2_ref->[$i] );
                return unless $reln;
                push @relns, $reln;
            }
            return \@relns;
